...
# ╔═╡ c4d161f2-a44d-11ef-1737-99f93dea22e1
using MultivariateStats, CairoMakie

# ╔═╡ 51452614-5e72-4d8a-bbca-05e41ae8d312
using DataFrames, CSV, DelimitedFiles

# ╔═╡ b4e7c6fe-f02a-430f-add5-bc65277b055a
using Impute

# ╔═╡ d4ccb3cc-4797-42b7-a532-93097149d653
md"# Example use of the GenomicDiversity.jl package"

# ╔═╡ f534e90e-1fc6-4302-ba76-5e8535bb7baf
pathToPackageFolder = "/Users/darrenirwin/Documents/Github_repos_MacBookPro/GenomicDiversity.jl/src/"

# ╔═╡ a7c894dc-2f90-4118-81fb-c8064d269471
begin
	include(string(pathToPackageFolder, "GenomicDiversity.jl"))
	using .GenomicDiversity
end

# ╔═╡ 385fa1e7-1ecb-4d0b-9ab8-ceafd6f1c952
greet_GenomicDiversity()

# ╔═╡ 8409a247-a1c6-4a86-a335-109db14c955c
pathToDemoData = "/Users/darrenirwin/Documents/Github_repos_MacBookPro/GenomicDiversity.jl/demoData/SparrowDemo_data_McCallumetal2024/"

# ╔═╡ 934b64d8-5f33-4692-8643-a161c94fbfba
begin
...
# ╔═╡ 96be2e21-fd80-478c-b4c0-0acc5b225e70
metadata = DataFrame(CSV.File(string(pathToDemoData, metadataFile)))

# ╔═╡ f863522b-20a3-40ed-8c81-59b97ae31c17
begin
    ind = DataFrame(CSV.File(string(pathToDemoData, individuals_file_name); header=["ind"], types=[String]))
    indNum = size(ind, 1) # number of individuals
end

# ╔═╡ 7f288a63-afbf-4636-b239-47b54efa4aa0
...
# ╔═╡ 5c49ed0c-bfc0-4e65-8685-bc21de081454
ind_with_metadata = hcat(ind, metadata)

# ╔═╡ d5b3e6fb-af16-4fd9-a780-9c59e77df477
pos_whole_genome = DataFrame(CSV.File(string(pathToDemoData, position_file_name); header=["chrom", "position"], types=[String, Int]))

# ╔═╡ af490fc7-b3d3-4c63-8710-dc7d2b011c74
begin # read in genotype data 
    geno = readdlm(string(pathToDemoData, genotype_file_name), '\t', Int16, '\n')
...
# ╔═╡ 7e6858f2-7ed6-4d53-8de1-641f5c95efdf
groups_to_plot = ["GCSP","PSWS","GWCS"]

# ╔═╡ efe6f5de-d057-4434-adeb-4032aa781ae8
group_colors = ["gold","red","blue"]

# ╔═╡ b3703c6c-91f6-4d9d-b21c-e97dd1d31c5c
begin
...
# ╔═╡ 9c54d4d0-9b65-42e8-82b3-5748a709d1d6
@time imputed_genos = Impute.svd(genosOnly_with_missing)

# ╔═╡ f068661d-7e65-417a-976a-6e06a15ded58
begin
...
# ╔═╡ 123509b9-b508-461d-86f1-dd22df944366
begin 
...
# ╔═╡ 103f1967-b41e-4fa4-a095-c5d82bdb2bb1
...
# ╔═╡ 48671ac6-08ab-4d2e-abdb-cdea1e3ea323
Fst, FstNumerator, FstDenominator, pairwiseNamesFst = GenomicDiversity.getFst(freqs,
                                            sampleSizes, groups_to_plot)

# ╔═╡ 8182c487-6322-4868-81f4-ae91e2be55a4
begin
    chr = "CM018231.2" # the name of a scaffold in the assembly
    regionInfo = GenomicDiversity.chooseChrRegion(pos_whole_genome, chr) #; positionMin=1) # this gets the maximum position for the chromosome
end

# ╔═╡ 8620267c-4c39-4b44-a2f8-42db78ccaf06
begin
...
# ╔═╡ 9223f097-6c6f-4be0-a1a4-fcf236686a89
# the end
